...
- `orchestrator/src/models/ark-client.mjs` — `maxTokens` param on `arkChat`
...
- Server simulation uses `LLM_*`
- Orchestrator generation uses `ARK_*`
- Generation can work while live simulation fails if only `ARK_*` is configured correctly
