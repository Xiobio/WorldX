---
name: worldx-generation-pipeline
...
- `contentLanguage` (`"zh"` | `"en"`) is output by the LLM based on the user's prompt language; carried through to `config/world.json` for runtime language hinting
- `originalPrompt` — the user's raw creation input — is passed from the orchestrator entry point into `config-generator.mjs` via an `options` parameter and stored in both `worldConfig.originalPrompt` and `worldConfig.metadata.originalPrompt`
...
- `contentLanguage` or `originalPrompt` not appearing in `config/world.json`
