...
- `sceneType`（字符串）：`"closed"` 或 `"open"`。closed：时间 24 小时循环，永不"打烊"，适合太空站、密室、海上漂流船等封闭空间。open：世界仅在活跃时段存在，到了关门/散场/下班时间即转场，适合有运营/活跃时段的场景（夜市、集市、酒吧、学校等），或各种常见场景（晚上各自回家睡觉了。如小镇、村庄）
- `timeConfig`（对象）：
